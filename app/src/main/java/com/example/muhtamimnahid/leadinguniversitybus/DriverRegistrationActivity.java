...
public class DriverRegistrationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        e4 = (EditText)findViewById(R.id.busnumber);
        ...
        auth.createUserWithEmailAndPassword(e2.getText().toString(),e3.getText().toString())
                .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if(task.isSuccessful())
                        {
                            Driver driver = new Driver(e1.getText().toString(),e2.getText().toString(),e3.getText().toString(),e4.getText().toString(),"24.869711","91.804752");
                            user = auth.getCurrentUser();
                            databaseReference = FirebaseDatabase.getInstance().getReference().child("LUBusDrivers").child(user.getUid());
